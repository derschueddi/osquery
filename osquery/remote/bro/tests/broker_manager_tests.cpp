...
TEST_F(BrokerManagerTests, test_announce) {
  ...
  auto msg = event.args();
  EXPECT_EQ(msg.size(), 3UL);
  // Node ID
  EXPECT_TRUE(broker::is<std::string>(msg[0]));
  auto peer_name = caf::to_string(get().ep_->node_id());
...
TEST_F(BrokerManagerTests, test_addandremovegroups) {
  ...
  broker::bro::Event test_msg1("message1", {broker::data("data1"), broker::data(42UL)});
  broker::bro::Event test_msg2("message2", {broker::data("data2"), broker::data(13.37)});
  ...
  EXPECT_EQ(msg1.size(), 2UL);
  EXPECT_EQ(msg2.size(), 2UL);
  // - Message 1
  EXPECT_TRUE(broker::is<std::string>(msg1[0]));
  EXPECT_EQ(broker::get<std::string>(msg1[0]), "data1");
  EXPECT_TRUE(broker::is<uint64_t>(msg1[1]));
  EXPECT_EQ(broker::get<uint64_t>(msg1[1]), 42UL);
  // - Message 2
  EXPECT_TRUE(broker::is<std::string>(msg2[0]));
  EXPECT_EQ(broker::get<std::string>(msg2[0]), "data2");
  EXPECT_TRUE(broker::is<double_t>(msg2[1]));
  EXPECT_EQ(broker::get<double_t>(msg2[1]), 13.37);
  ...
  EXPECT_NE(std::find(topics.begin(), topics.end(), get().TOPIC_ALL),
            topics.end());
  EXPECT_NE(std::find(topics.begin(),
                      topics.end(),
                      get().TOPIC_PRE_INDIVIDUALS + get().getNodeID()),
            topics.end());
  EXPECT_NE(
      std::find(topics.begin(), topics.end(), get().TOPIC_PRE_GROUPS + "test1"),
      topics.end());
  ...
  EXPECT_NE(subscribers.find(get().TOPIC_PRE_INDIVIDUALS + get().getNodeID()),
            subscribers.end());
  EXPECT_NE(subscribers.find(get().TOPIC_PRE_GROUPS + "test1"),
            subscribers.end());
